...
nicht überschreitet - unabhängig von der Anfahrtszeit pro Positionseinheit (zwei
getrennte Größen: wie teuer eine Fahrt ist, ist nicht dasselbe wie wie weit ein
Agent kommunizieren kann). Das ist der einzige Punkt in dieser Demo, an dem "mehrere
Agenten" tatsächlich etwas anderes bedeutet als "eine zentrale lokale Suche" - ohne
dieses Gate würde die Verhandlung mechanisch exakt einer zentralen Lokalsuche
entsprechen, nur mit Agenten-Vokabular.
...
gilt - unter Verwendung der Startpositionen und der rohen physischen Distanz, nicht
der (durch `travel_time_per_unit` skalierten) Anfahrtszeit. Das trennt zwei
unabhängige Größen: wie teuer eine Fahrt ist, und wie weit ein Agent überhaupt
kommunizieren kann.
